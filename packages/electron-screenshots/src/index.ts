...
export default class Screenshots extends Events {
   ...
   * 开始截图
   */
  public async startCapture(options = {}): Promise<void> {
   ...
   * 设置全局参数
   */
  public async setGlobalData(): Promise<void> {
    this.logger('setGlobalData', this.viewOptions);
    this.$view.webContents.send('SCREENSHOTS:setGlobalData', this.viewOptions);
  }
  ...
  private listenIpc(): void {
     ...
     * 翻译事件
     */
    ipcMain.on('SCREENSHOTS:translate', (e, buffer: Buffer, data: ScreenshotsData) => {
      this.logger(
        'SCREENSHOTS:translate buffer.length %d, data: %o',
        buffer.length,
        data,
      );
