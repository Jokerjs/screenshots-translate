...
contextBridge.exposeInMainWorld('screenshots', {
  ...
  translate: (arrayBuffer: ArrayBuffer, data: ScreenshotsData) => {
    console.log('contextBridge translate', arrayBuffer, data);

    ipcRenderer.send('SCREENSHOTS:translate', Buffer.from(arrayBuffer), data);
  },
  translateError: (error: any) => {
    console.log('error translate', error);
    ipcRenderer.send('SCREENSHOTS:translateError', error);
  },
