...
export default memo(
  forwardRef<CanvasRenderingContext2D>(function ScreenshotsCanvas (
    ...
    const { url, image, width, height, translatedImage } = useStore()
    ...
    return (
      ...
      >
        ...
        >
          {
            translatedImage?.loading && (
              <Spin tip='翻译中...' wrapperClassName='screenshots-canvas-translate-loading'>
                <div />
              </Spin>
            )
          }
          {
            translatedImage?.text && (
              <div className='screenshots-canvas-translate-text'>{translatedImage?.text}</div>
            )
          }
          {
            translatedImage?.base64 && (
              <img src={translatedImage?.base64} className='screenshots-canvas-translate-base64' alt='' />
            )
          }
          {isCanResize && (
